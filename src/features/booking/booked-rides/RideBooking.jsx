...
export default function RideBooking({ rideDetails, totalSeat }) {
  ...
  useEffect(() => {
    if (ride) {
      setRidePassed(isRidePassed(ride?.ride_date, ride?.departure_time));
    }
  }, [ride]);
  ...
  if (!ride) {
    return (
      <div className="ride-not-found">
        <p className="error-happens">Ride data not found.</p>

        <button className="btn register-btn" onClick={() => router.back()}>
          Go Back
        </button>
      </div>
    );
