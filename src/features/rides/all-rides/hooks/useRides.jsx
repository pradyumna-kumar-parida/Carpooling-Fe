...
export function useRides({ from, to, date, passengers }) {
  ...
  const filtered = rides.filter((ride) => {
    if (verifiedOnly && ride.driver_is_verified !== "1") return false;

    if (departSlot && getDepartSlot(ride.departure_time) !== departSlot)
      return false;

    if (amenityChecks.instant_booking && ride.instant_booking !== "yes")
      return false;

    if (amenityChecks.smoking_allowed && ride.smoking_allowed !== "yes")
      return false;

    if (amenityChecks.pet_allowed && ride.pet_allowed !== "yes") return false;

    if (amenityChecks.max_two_in_back && ride.max_two_in_back !== "yes")
      return false;
